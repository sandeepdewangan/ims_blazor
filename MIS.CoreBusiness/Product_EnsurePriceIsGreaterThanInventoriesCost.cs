    ...
    public class Product_EnsurePriceIsGreaterThanInventoriesCost : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            if (product is not null)
            {
                ...
                {
                    return new ValidationResult($"The product price is less than the inventories cost: {TotalInventoryCost(product).ToString("c")}!", new List<string>() { validationContext.MemberName });
                }
            }
